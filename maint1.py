...
# Function to generate a random HMAC secret key
def generate_secret_key():
    return secrets.token_bytes(32)  # Generate a 32-byte (256-bit) random key

# Function to compute HMAC hash with salting
def compute_hmac_hash(data, secret_key, salt):
...
sample_path = "C:\\Users\\prana\\Downloads\\Fingerprint Recognition Project\\SOCOFing\\Altered\\Altered-Hard\\118__F_Right_little_finger_Obl.BMP"
...
# Generate a random HMAC secret key and salt
...
for file in os.listdir("C:\\Users\\prana\\Downloads\\Fingerprint Recognition Project\\SOCOFing\\Real")[:1000]:
    ...
    # Compute HMAC hash values for the sample and fingerprint images with salting
    sample_hash = compute_hmac_hash(sample.tobytes(), secret_key, salt)
    fingerprint_hash = compute_hmac_hash(fingerprint_image.tobytes(), secret_key, salt)

    # Compare the HMAC hash values
    if sample_hash == fingerprint_hash:
        # If the HMAC hashes match, consider it a perfect match
        score = 100
    ...
    if score > best_score:
        ...
        filename = file
        best_matching_hash = fingerprint_hash
        kp1, kp2, mp = keypoints_1, keypoints_2, good_matches

print("BEST MATCH:", filename)
print("Score:", best_score) 
print("Hash of Best Matching Image:", best_matching_hash)
